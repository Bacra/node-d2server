var http = require('http'),
	...
	downServ = require('./i2serv.js').infoServ,
	DocumentRoot = require('../../conf.js').DocumentRoot,

	urlReg = /^\/download/i,
	_formBuf = fs.readFileSync(__dirname+'/../src/downform.html');
	...
	if (req.method == "POST") {
		...
		req.on('data', function (chunk) {
			...
			var postDataObject = querystring.parse(postData),
				leaveFileNum = 0,
				root = DocumentRoot,
				appname = postDataObject.appname && postDataObject.appname.trim();

			if (appname) {
				root += appname;
				if (postDataObject.init) {
					require('./deployApp.js')(postDataObject.appname, 'simple');
				}
			}
