...
module.exports = function(confDataAPI, projConf){
	...
	return function(uri, res, req){
		var mainQuery = util.query(),
			query = util.cloneObject(uri.query),
		...
		mainQuery.join(function(){
			var dataDirname = getDateDirname(uri.pathname),
				dataFilename = getDateFilename(query),
			...
			if (isPost) dataFilename = '[POST] '+dataFilename;
			...
			var cacheDateFile = dataDirname+'[CACHE] '+dataFilename;

			// 获取返回内容
			if (confDataAPI.update || (confDataAPI.cache && !util.fileExists(dataFile) && !util.fileExists(cacheDateFile))) {
					...
					wget({
						...
						if (res2.statusCode == 200) {
							wget.getBuffer(res2, function(buf){
								...
								sendData(buf);
								if (confDataAPI.cache) util.writeFile(cacheDateFile, buf);
							}, function(err){
								getDataFail(res);
							});
						} else {
