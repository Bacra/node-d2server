	...
	if (cont.fileMap) {
		...
		util.eachObject(cont.fileMap, function(oFile, srcFiles){
			...
			fileMap.push(getConvertReg(baseExtname)(oFile), srcFiles.map(function(v){
				...
				if (_testReg.test(basename)) {
					myTestLink.push(srcFile);
				} else {
					mySourceLink.push(srcFile);
				}
			...
			if (mySourceLink.length) sourceLink[oFile] = mySourceLink;
			if (myTestLink.length) testLink[oFile] = myTestLink;
		});
