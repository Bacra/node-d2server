	...
	if (cont.HTML) {
		...
		util.eachObject(cont.HTML, function(srcFile, oFiles){
			...
			util.eachObject(oFiles, function(href, fileConf){
				...
				var virtualFile = util.parsePath(root+href);
				sourceMap[virtualFile] = srcFile;

				// 处理配置的参数
				projConf.initHTMLConfig(fileConf);
				fileConf.href = href;
				fileConf.htmlVirtualFileDirname = path.dirname(virtualFile)+path.sep;
		...
		this.convertSource4HTML = function(cont, htmlVirtualFileDirname){
			var src = {},
				convertFunc = function(str, open, split, srcPath, close){
					...
					var srcFile = util.parsePath(srcPath[0] == '/' || srcPath[0] == '\\' ? projConf.root + srcPath : htmlVirtualFileDirname+srcPath);
