...
function ProjConfigClass(dirname, cont) {
	...
	if (cont.HTML) {
		...
		util.eachObject(cont.HTML, function(srcFile, oFiles){
			...
			// header footer
			var header = projConf.defaultHeader;
			var footer = projConf.defaultFooter;
			if ('header' in oFiles) {
...
function watchConfCh(file, projConf){
	...
	return watchFileCh(file, function(){
		...
		if (fs.existsSync(mainFile)) {
			var rs = reloadProjConfig(projConf.dirname, ProjConfigClass);
			if (rs) {
				projConf = rs;
...
fs.watch(_conf.DocumentRoot, {persistent: false}, function(e, dirname){
	if (_watchDirnameTimer[dirname]) {
		clearTimeout(_watchDirnameTimer[dirname]);
	} else if(e != 'rename'){
		return;
	}
...
function initProjConfig(dirname){
	...
	fs.exists(file, function(exists){
		if (exists) reloadProjConfig(dirname, ProjConfigClass);
	});
}
