...
function ProjConfigClass(dirname, cont) {
	...
	if (cont.HTML) {
		...
		util.eachObject(cont.HTML, function(srcFile, oFiles){
			...
			util.eachObject(oFiles, function(href, fileConf){
				sourceMap[util.parsePath(root+href)] = srcFile;
				...
				fileConf.href = href;
				fileConf.htmlVirtualFileDirname = path.dirname(util.parsePath(root+href))+path.sep;

				HTMLLinks[href] = fileConf.title;

				fDatas[util.parsePath(root+href)] = fileConf;
			});
	...
	if (cont.fileMap) {
		...
		util.eachObject(cont.fileMap, function(file, fileConf){
			var basePath = catalog + path.dirname(fileConf.target) + '/',
				mySourceLink = [];
			
			convertConf[file] = fileConf.srcFiles.map(function(v){
				...
				sourceMap[util.parsePath(root+basePath+virtualFilename)] = srcFile;		// fileServ 地址转化
			...
			if (mySourceLink.length) {		// 拒绝空数组保存（可在split过程中少进行一次if判断）
				sourceLink[util.parsePath(root+fileConf.target)] = mySourceLink;
			}
		});
		...
		this.convertSource4HTML = function(cont, htmlVirtualFileDirname){
			var src = {},
				convertFunc = function(str, open, split, srcPath, close){
					var srcFile = util.parsePath(htmlVirtualFileDirname+srcPath);
					if (convertConf[srcFile]) {
						var dirname = path.dirname(srcPath)+'/';
						return convertConf[srcFile].map(function(virtualFile){
							return open+dirname+virtualFile+close;
						}).join('\n');
					}
					return str;
				};
