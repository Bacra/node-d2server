...
function ProjConfigClass(dirname, cont) {
	...
	if (cont.HTML) {
		...
		util.eachObject(cont.HTML, function(srcFile, oFiles){
			...
			util.eachObject(oFiles, function(virtualFile, fileConf){
				sourceMap[virtualFile] = srcFile;

				// 处理配置的参数
				projConf.initHTMLConfig(fileConf);
				fileConf.htmlVirtualFileDirname = path.dirname(virtualFile)+path.sep; 		// 解析script link资源需要（转化为绝对路径）
				HTMLLinks[fileConf.href] = fileConf.title;

				fDatas[virtualFile] = fileConf;
			});
	...
	// baseLess 文件
	if (cont.baseLess) {
		var baseLess = this.baseLessFile = util.parsePath(root + _conf.SourcePath+ cont.baseLess);

		if (util.fileExists(baseLess)) {
			this.reloadBaseLess();

			if (this.watchFiles[baseLess]) this.watchFiles[baseLess].close();
