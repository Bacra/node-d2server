...
function ProjConfigClass(dirname, cont) {
	...
	if (cont.HTML) {
		...
		util.eachObject(cont.HTML, function(virtualFile, fData){
			srcFile = sourceRoot + fData.srcFile;
			...
			fData[virtualFile] = fData;
			HTMLData[srcFile] = fData;
		});
	...
	// baseLess 文件
	if (cont.baseLess) {
		var baseLess = this.baseLessFile = path.normalize(sourceRoot + cont.baseLess);
...
ProjConfigClass.prototype = {
	...
	'getHTMLConfig': function(file, outFile){
		var data, fData = this.HTMLData[file];
		if (fData) {
			if (outFile) {
				data = fData[outFile];
				if (data) return data;
