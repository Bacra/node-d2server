...
module.exports = function(dirname, callback){
	...
	try {
		...
		// 初始化资源变量
		var catalog = mainConf.catalog || '',
			servRoot = root+catalog;
		...
		if (mainConf.HTML) concatHTMLConf(mainConf.HTML, HTML, servRoot, sourceRoot, catalog, mainConf.defaultHeader, mainConf.defaultFooter);
		mainConf.HTML = HTML;
		...
		if (mainConf.extra) {
			...
			mainConf.extra.map(function(v){
				...
				// 合并HTML配置
				if (conf.HTML) concatHTMLConf(conf.HTML, HTML, servRoot, sourceRoot, catalog, mainConf.defaultHeader, mainConf.defaultFooter);
...
function concatHTMLConf(conf, RsConf, servRoot, sourceRoot, catalog, defaultHeader, defaultFooter){
	if ('header' in conf) {
	...
	util.eachObject(conf, function(srcFile, outConf){
		srcFile = util.parsePath(sourceRoot + srcFile);

		var htmlConf = RsConf[srcFile],
		...
		if (!htmlConf) RsConf[srcFile] = htmlConf = {};
		util.eachObject(outConf, function(file, data){
			data.href = catalog+file;
			file = util.parsePath(servRoot+file);
			if (htmlConf[file]) throw new reloadError('Can not override "'+file+'" in HTML Config', srcFile);
			htmlConf[file] = data;
		});
	});
}
