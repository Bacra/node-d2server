...
module.exports = function(dirname, callback){
	...
	try {
		...
		// 初始化资源变量
		var servRoot = mainConf.catalog ? root+mainConf.catalog : root;
...
function concatFileMapConf(conf, RsConf, servRoot) {
	util.eachObject(conf, function(target, srcArr){
		var file = util.parsePath(servRoot+target),
			fileConf = RsConf[file];
		if (fileConf) {
			fileConf.srcFiles = fileConf.srcFiles.concat(srcArr);
		} else {
			RsConf[file] = {
				'target': target,
				'srcFiles': srcArr
