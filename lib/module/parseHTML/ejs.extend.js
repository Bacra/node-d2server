var ejsFile = './d2server-ejs/ejs.js',
	ejs = require(ejsFile),
	fs = require('fs'),
...
// 删除在require中的副本，让其他引用ejs的方法，使用原生的ejs
delete require.cache[require.resolve(ejsFile)];


ejs.onParseMethod = function(code){
	code = code.trim();
	var match;
	if (code == 'endLoop') {
		return '})';
		...
		return 'if (!sys.blockReg || sys.blockReg.test("'+match[1]+'")) {';
	}
 ...
 */
ejs.onIncludeTpl = function(inFile, parentFile, options){
