...
fs.readdirSync(_exampleRoot).forEach(function(v){
	...
	if (!fs.existsSync(confFile)) {
		notice.warn('Object Init', 'The plan('+v+') do not has projconf file');
		return;
	}
...
function readDir(root, path, projTpl){
	...
	for(var existsPath in projTpl)
	{
		if (existsPath.indexOf(path) != -1)
		{
			return;
		}
	}
...
module.exports = function(projName, plan){
	...
	if (!util.fileExists(confFile)) {
		...
		if (!plan || !_projTpls[plan]) {
			notice.log('Object Init', 'The plan('+plan+') switch to `default`');
		...
		try {
			util.eachObject(plan, function(i, v){
				...
				if (fs.existsSync(file)) {
					notice.warn('Object Init', 'The copying file is exists', file);
				} else if (v === null) {
					util.mkdirs(file);
				} else {
					var cont = v.toString();
					if (_projnameTestReg.test(cont)) v = cont.replace(_projnameReg, projName);
					util.writeFile(file, v);
				}
			});
			...
			// 初始化项目
			// if (ProjConfig.reload(projName)) notice.log('Object Init', 'Object create success');
		} catch(err) {
			notice.error('', err);
		}
	} else {
