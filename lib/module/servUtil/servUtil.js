	...
	return function(req, res){
		...
		if (projConf) {
			if (projConf.sourceMap[srcFile]) {
				...
				return;
			} else if (srcFile.indexOf(SourcePath) != -1) {
				res.writeHead(403, {
				...
				srcFile = projConf.root + srcFile.substring(projConf.root.length + projConf.catalog.length);			// 粗暴地拿到新地址，即使地址有问题，也没关系，无法找到相应的文件就会转到dataAPI下
			}
		} else {
