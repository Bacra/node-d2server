...
module.exports = function(projConf){
	...
	util.eachObject(projConf.sourceLink, function(outFile, linkFiles){
		...
		var outFileExtname = util.getExtname(outFile),
			...
			query = Query(mainTask.completeDR),
			_cont;

	
		if (outFileType == 'js' || outFileType == 'css' || outFileType == 'less') {
			if (outFileType == 'js') {
				query.add(function(nextFunc){
					comboFile(linkFiles, function(cont){
						_cont = cont;
						nextFunc();
					}, '\n\n\n');
				});
			...
			if (outFileType == 'js') {
				query.add(function(nextFunc, endFunc){
			...
			query.add(function(nextFunc, endFunc){
				comboFile(linkFiles, function(buf){
					_cont = buf;
					nextFunc();
				});
			});
		...
		mainTask.add(query.start.bind(query));
	});
	...
	util.eachObject(projConf.HTMLData, function(virtualFile, fData){
		var query = Query(mainTask.completeDR),
