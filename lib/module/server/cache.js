var fs = require('fs'),
	...
	parseHTML = require('../parser/parseHtml.js').parse4serv,
	parseLess = require('../parser/parseLess.js');



function htmlOptions(appConf, file, mark) {
	var data, HTMLData = appConf.HTMLData[file];

	if (mark && HTMLData) {
		data = HTMLData[mark];
		if (!data) data = HTMLData['default'];
	} else if (HTMLData){
		data = HTMLData['default'];
	} else {
	...
	if (extname == 'html' || extname == 'less') {			// 需要解析的内容
		...
		if (extname == 'less'){
			...
			getContData = createGetContData(parseHTML, function(mark){
				return htmlOptions(appConf, file, mark);
			});
		}
