...
function htmlOptions(appConf, mark) {
	...
	if (mark) {
		HTMLData = appConf.HTMLData[mark];
	...
	if (extname == 'html' || extname == 'less') {			// 需要解析的内容
		...
		sendCont = function(res, mark){
			getContData(function(err, buf){
				directSendData(res, extname.toUpperCase(), err, buf, getHead);
			}, mark);
		};
	} else if (drGetCont) {
...
// 直接发送数据
function directSendData(res, errorName, err, buf, getHead){
	if (err) {
		res.statusCode = 500;
		res.end();
