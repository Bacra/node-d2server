...
function AppConfig(root, cont) {
	...
	// HTML页面配置信息处理
	this.defaultHeader = getIncludeTpl(cont.defaultHeader, '', true);
	this.defaultFooter = getIncludeTpl(cont.defaultFooter, '');
	if (cont.HTML) {
		var HTMLData = {};
		util.eachObject(cont.HTML, function(srcFile, oFiles){
			...
			util.eachObject(oFiles, function(i, fData){
				...
				if(!fs.existsSync(redirectFile)) {
					util.writeFile(redirectFile, '<script type="text/javascript">window.location.href="'+urlPrefix+i+'"</script>');
				}
				

				// 处理配置的参数
				// fData = Object.create(fData);
				fData = self.initHTMLConfig(fData);
				blocks[fData.block || 'default'] = fData;
			});

			if (!('default' in blocks)) blocks['default'] = self.initHTMLConfig({});
...
AppConfig.prototype = {
	...
	'initHTMLConfig': function(fData) {
		...
		fData.header = getIncludeTpl(fData.header, this.defaultHeader, true);
		fData.footer = getIncludeTpl(fData.footer, this.defaultFooter);
	...
	'getHTMLConfig': function(file, blockStr){
		var data, HTMLData = this.HTMLData[file];

		if (HTMLData) {
			if (blockStr) {
				data = HTMLData[blockStr];
