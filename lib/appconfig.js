...
function AppConfig(root, cont) {
	this.root = root;			// 必须是已经经过parsePath处理之后的结果
	...
	fs.exists(root+'.d2server.html', function(exists){
		if (!exists) util.writeFile(root+_conf.RedirectFile, '<script type="text/javascript">window.location.href="'+urlPrefix+'"</script>');
	});
...
function reloadAppConfig(root, isInit){
	var mainFile = root + _conf.MainConfigFile,
		mainPath, mainConf,
	...
	appConf = new AppConfig(root, mainConf);
...
fs.readdir(_conf.DocumentRoot, function(err, files) {
	if (err) notice.error('SYS', 'Serv Document not exists', _conf.DocumentRoot);
	files.forEach(function(v){
		if (v != '.'&& v != '..') {
			var file = _conf.DocumentRoot +v+'/'+_conf.MainConfigFile;
			fs.exists(file, function(exists){
				if (exists) reloadAppConfig(util.parsePath(_conf.DocumentRoot +v+'/'), true);
			});
		}
	});
});
