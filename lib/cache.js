	...
	if (extname == 'html' || extname == 'less') {			// 需要解析的内容
		...
		if (extname == 'less'){
			...
			var createGetContData = function(parseFunc, getOptionsFunc){
				...
				return function(finalOutCallback, outFile){
					// 由于outfile不指明，或则相应的opts不存在，都不会执行到此过程
					// 所以可以删除这一块的验证
					if (outFile) {
						var func = getContDataFuncs[outFile];
						if (!func) {
							var opts = getOptionsFunc(outFile);
							if (!opts) {
								notice.warn('Cache', 'outFile is not exists', file);
								finalOutCallback('outFile is not exists', '');
								return;
							}
							func = buildGetContDate(parseFunc, opts);
							getContDataFuncs[outFile] = func;
						}
						func(finalOutCallback);
					} else {
						notice.warn('Cache', 'outFile is not specified', file);
						finalOutCallback('outFile is not specified', '');
					}
				};
			};

			getContData = createGetContData(parseHTML, function(outFile){
				return appConf.getHTMLConfig(file, outFile);
			});
		}
