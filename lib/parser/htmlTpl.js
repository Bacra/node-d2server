...
ejs.parseMethodExtFunc = function(code){
	...
	if (code == 'endLoop') {
		...
		return 'if (!block || block.test("'+blocks+'")) {';
	}
...
ejs.includeExtFunc = function(inFile, parentFile){
	inFile = util.parsePath(inFile);
	parentFile = util.parsePath(parentFile);

	if (!AppConfig) AppConfig = require('../appconfig.js');		// Cache和appconfig有调用的先后顺序

	var appConf = AppConfig.find(parentFile),
