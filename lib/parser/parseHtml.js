...
module.exports = {
	'parse4file': function(cont, file, options){
		return ejs.compile(htmlTpl(options.header + cont + options.footer, file, options, parseHTML), {'filename': file, 'cache': false})(options.data);
	},
	'parse4serv': function(cont, file, callback, errorCallback, options){
		try {
			callback(parseHTML(options.header + cont + options.footer + getIoScriptStr(options.appConf.root), file, options));
		} catch (err) {
			errorCallback(err);
		}
	}
